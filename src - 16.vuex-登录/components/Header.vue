...
<div>
  <nav class="site-header sticky-top py-1">
      <div class="container d-flex flex-column flex-md-row justify-content-between">
        <router-link class="py-2" exact active-class="active" to="/">
        ...
        <router-link class="py-2 d-none d-md-inline-block" exact active-class="active" to="/restaurant">餐厅查询</router-link>
        ...
        <template v-else>
           <p class="currentUser text-light">{{currentUser}}&nbsp;&nbsp;&nbsp;&nbsp;<span class="text-primary" @click="clearLogin">[退出]</span></p>
        </template>
      </div>
    </nav>
</div>
...
<style  scoped>
  ...
  .active{color:#007bff;}
  .currentUser{margin-top:10px;margin-right: 10px;}
</style>